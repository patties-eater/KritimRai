    ...
    <main>
      ...
      {featuredService ? (
        <section className="section-shell">
          <div className="grid gap-8 rounded-[30px] border border-[rgba(68,42,24,0.09)] bg-[linear-gradient(135deg,rgba(255,248,241,0.96),rgba(240,226,211,0.88))] p-7 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              <p className="eyebrow">Most popular package</p>
              <h2 className="section-title">{featuredService.name}</h2>
              <div className="mt-4 inline-flex items-center rounded-full bg-[var(--color-ink)] px-5 py-3 shadow-[0_12px_30px_rgba(45,28,19,0.18)]">
                <p className="font-display text-2xl text-[var(--color-paper)] sm:text-3xl">
              ...
              <p className="mt-4 max-w-2xl text-[var(--color-muted)]">{featuredService.description}</p>
              <p className="mt-4 text-[var(--color-accent-deep)]">{featuredService.deliverables}</p>
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {featuredService.features.map((feature) => (
                  <div
                    key={feature}
                    className="rounded-2xl border border-[rgba(61,41,28,0.12)] bg-[rgba(255,255,255,0.62)] px-4 py-3 text-sm text-[var(--color-ink)]"
                  >
                    {feature}
                  </div>
                ))}
            ...
            <div className="grid content-center gap-4">
              ...
              <Link className="button button-primary w-fit" to="/contact">
                Book this package
              </Link>
              <Link
                className="inline-flex w-fit items-center rounded-full border border-[rgba(61,41,28,0.12)] bg-[rgba(255,255,255,0.78)] px-5 py-3 text-[var(--color-ink)] shadow-[0_10px_25px_rgba(45,28,19,0.08)] transition duration-200 hover:-translate-y-0.5"
          ...
          {otherServices.map((service) => (
            <article key={service.name} className="info-card">
              <div className="flex items-start justify-between gap-4">
                <div className="rounded-2xl bg-[rgba(199,138,82,0.14)] px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.7)]">
                  <p className="font-display text-xl leading-none text-[var(--color-accent-deep)]">
                    {service.price}
                  </p>
                </div>
                <span className="rounded-full border border-[rgba(61,41,28,0.12)] px-3 py-1 text-xs text-[var(--color-accent-deep)]">
                  {service.duration}
                </span>
              </div>
              ...
              <ul className="mt-4 grid gap-2 text-sm text-[var(--color-ink)]">
              ...
              <Link
                className="mt-5 inline-flex w-fit items-center rounded-full border border-[rgba(199,138,82,0.3)] bg-[rgba(199,138,82,0.12)] px-5 py-3 font-medium text-[var(--color-ink)] shadow-[0_10px_20px_rgba(199,138,82,0.12)] transition duration-200 hover:-translate-y-0.5 hover:bg-[rgba(199,138,82,0.2)]"
